...
async def update_document(
    ...
    Note: Changes are reversible by calling update_document again with the original values.
    For task-friendly updates with named parameters, use update_task instead.
    """
    start_time = time.time()
    try:
        api_client = create_api_client()
...
async def create_goal(
    ...
    try:
        ...
        if not isinstance(stored, dict) or stored.get("_id") != goal_id:
            logger.error("Goal %s was accepted but not stored", goal_id)
            return create_error_response(
                RuntimeError(
                    f"Goal {goal_id} was not stored — the create request was "
                    "accepted but the document cannot be read back."
                ),
                "/doc/create",
                debug,
                start_time,
...
async def create_recurring_task(
    ...
    start_in: int | None = None,
    end_in: int | None = None,
    end_date: str | None = None,
    task_time: str | None = None,
    limit_to_weekdays: bool = False,
    descendants: list[dict[str, Any]] | None = None,
    debug: bool = False,
) -> StandardResponse:
    """Create a recurring project template (requires AMAZING_MARVIN_FULL_ACCESS_TOKEN).

    Args:
        title: Template title.
        recurrence_type: Recurrence pattern — 'daily', 'repeat week', 'n per week',
            'monthly', 'repeat month', 'repeat year', 'echo' or 'custom'.
        repeat_start: Anchor date YYYY-MM-DD. Occurrences are computed from it;
            the anchor is not advanced as they are generated.
        repeat: Interval, e.g. 2 with 'repeat year' means every second year.
        parent_id: Owning category ID.
        note: Markdown note, copied to every occurrence.
        label_ids: Label IDs to attach.
        time_estimate: Time estimate in milliseconds.
        due_in: Days from each occurrence to its due date.
        start_in: Days offset for the occurrence's start date.
        end_in: Days offset for the occurrence's end date.
        end_date: YYYY-MM-DD after which nothing is generated.
        task_time: HH:MM time of day on generated occurrences.
        limit_to_weekdays: Skip weekends when generating.
        descendants: Template children, each a Tasks-shaped dict. Every
            occurrence receives a copy.

    The calendar fields Marvin stores alongside the anchor (day, date,
    weekDays) are derived from repeat_start.

    Occurrences are generated by the Marvin client, not by the server, and the
    automatic pass is unreliable — ask the user to press the generate button in
    the recurring-tasks view instead of waiting for it. Never build an
    occurrence by hand: its ID is derived from the template and would collide
    with the one the client produces.
    """
    start_time = time.time()
    try:
        api_client = create_api_client()
        template_req = RecurringTaskCreateRequest(
            title=title,
            type=recurrence_type,  # type: ignore[arg-type]
            ...
            end_date=end_date,
            task_time=task_time,
            limit_to_weekdays=limit_to_weekdays,
            descendants=descendants or [],
        )
        document = template_req.to_document()
        template_id = document["_id"]
        api_client.create_recurring_task(document)

        stored = api_client.get_document(template_id)
        if not isinstance(stored, dict) or stored.get("_id") != template_id:
            logger.error("Template %s was accepted but not stored", template_id)
            return create_error_response(
                RuntimeError(
                    f"Template {template_id} was not stored — the create request "
                    "was accepted but the document cannot be read back."
                ),
                "/doc/create",
                debug,
                start_time,
        ...
        return create_simple_response(
            data={"created_recurring_task": stored},
            summary_text=(
                f"Created recurring template: {title}. Occurrences appear once the "
                "Marvin client generates them — use the generate button rather than "
...
async def update_task(
    ...
    """Update a task using friendly named parameters (requires AMAZING_MARVIN_FULL_ACCESS_TOKEN).

    Pass only the fields you want to change; omitted fields (None) are left unchanged.
    Note: there is no way to *clear* a field (e.g. remove a due date) via this tool —
    use update_document with an explicit setter value instead.

    Args:
        item_id: The task's _id
        title: New title
        due_date: Due date in YYYY-MM-DD format
        scheduled_date: Scheduled/day date in YYYY-MM-DD format (maps to Marvin 'day' field)
        note: Task notes/description
        label_ids: List of label IDs to assign
        priority: Priority value
        parent_id: Parent project/category ID
        is_starred: Whether the task is starred
        is_frogged: Whether the task is frogged (urgent)
        time_estimate: Time estimate in minutes
        backburner: Whether to put the task on the backburner
    """
    start_time = time.time()
    try:
        api_client = create_api_client()
...
async def record_habit(
    ...
    """Record a habit as done for today.

    Args:
        habit_id: The habit's _id
        value: Optional numeric value for quantitative habits (e.g. steps, minutes)
    """
    start_time = time.time()
    try:
        api_client = create_api_client()
...
async def undo_habit(habit_id: str, debug: bool = False) -> StandardResponse:
    """Undo the most recent recording of a habit.
...
async def add_event(
    ...
    """Add a calendar event to Amazing Marvin.

    Args:
        title: Event title
        start: Start datetime (ISO 8601 or timestamp the API accepts)
        length_minutes: Event duration in minutes
...
async def get_today_time_blocks(
    ...
    """Get time blocks scheduled for today (or a specific date).
...
async def set_reminders(
    ...
    Args:
        reminders: List of reminder objects (see Amazing Marvin API docs for schema)
    """
    start_time = time.time()
    try:
        api_client = create_api_client()
...
async def unclaim_reward_points(
    ...
    """Unclaim previously claimed reward points for a task.

    Args:
        item_id: The task's _id
        date: Date the points were claimed, in YYYY-MM-DD format
    """
    start_time = time.time()
    try:
        api_client = create_api_client()
